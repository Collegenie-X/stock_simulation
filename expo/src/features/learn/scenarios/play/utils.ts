...
export function getInitialState(stock: StockInfo): GameState {
  // 시작 자금의 일부를 주식으로 들고 시작 → 1턴부터 "팔래"도 선택 가능
  const budget = G.defaultCash
  const startQty = G.defaultHoldings + (stock.initialPrice > 0 ? Math.floor((budget * G.startStockRatio) / stock.initialPrice) : 0)
  const initialCash = budget - (startQty - G.defaultHoldings) * stock.initialPrice

  return {
    cash: initialCash,
    holdings: startQty,
    avgPrice: startQty > 0 ? stock.initialPrice : 0,
    totalInitial: initialCash + startQty * stock.initialPrice,
...
export function analyzePersonality(trades: TradeRecord[]): PersonalityResult {
  ...
  for (const trade of trades) {
    ...
    const sec = trade.decisionTimeSec ?? 8
    ...
    if (sec <= G.timeBonus.fast.sec) {
      scores.challenger += 1
      scores.emotional += 1
    } else if (sec >= G.timeBonus.normal.sec) {
      scores.analyst += 1
      scores.systematic += 1
    }
  }
