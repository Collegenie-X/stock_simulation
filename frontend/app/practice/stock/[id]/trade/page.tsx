      ...
      <div className={cn(
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-sm font-bold text-white truncate">{stockName}</span>
          </div>
        ...
        {/* 체크 */}
        <div className="shrink-0 text-green-400 text-sm">✓</div>
      </div>
    </div>
  )
}
...
export default function TradePage() {
  ...
  const handleAction = () => {
    ...
    console.log("✅ 저장 성공 - 거래 완료")

    // 거래 결과를 sessionStorage에 저장 (리스트 화면에서 토스트로 표시)
    if (orderType === "market") {
      ...
      const profitRate = !isBuy && avgBuyPrice > 0
      ...
      sessionStorage.setItem("lastTradeToast", JSON.stringify(tradeToast))
    }
