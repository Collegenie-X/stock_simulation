...
4. [🏆 랭킹 & 경쟁 시스템](#-랭킹--경쟁-시스템)
5. [📊 보고서 시스템](#-보고서-시스템)
6. [📝 연습 문제 (난이도별)](#-연습-문제-난이도별)
7. [🔔 스마트 알림 시스템](#-스마트-알림-시스템)
8. [💡 AI 힌트 시스템](#-ai-힌트-시스템)
9. [⚙️ 게임 알고리즘 및 순서도](#️-게임-알고리즘-및-순서도)
10. [📅 최신 데이터 활용](#-최신-데이터-활용)
11. [🌊 하루 파도 변화 단계](#-하루-파도-변화-단계)
12. [🎮 게임 효과 & 특수성](#-게임-효과--특수성)
13. [시스템 아키텍처](#시스템-아키텍처)
...
게임 중 중요한 순간을 놓치지 않도록 **자동 알림**이 표시됩니다!
...
| 🚨 알림 유형 | 📊 조건 | ⏱️ 타이밍 | 🎯 추천 행동 |
|:---:|:---:|:---:|:---:|
| **📉 -5% 급락** | 5% 이상 하락 | 즉시 | 손절 고려 |
| **📈 +5% 급등** | 5% 이상 상승 | 즉시 | 익절 고려 |
| **💎 최저가 도달** | 52주 최저가 경신 | 즉시 | 매수 기회 |
| **🚀 최고가 도달** | 52주 최고가 경신 | 즉시 | 익절 타이밍 |
| **⚡ 거래량 폭발** | 평균 대비 200%+ | 실시간 | 변동성 주의 |
| **🛡️ 지지선 근접** | 지지선 -2% 이내 | 즉시 | 매수/손절 판단 |
| **🔥 저항선 돌파** | 저항선 +2% 돌파 | 즉시 | 추가 상승 가능 |
| **🤖 AI 동작** | AI 매수/매도 시 | 2초 이내 | AI 따라하기 |
...
- **게임 (기본)**: http://localhost:3000/practice/stock/scenario-1
- **게임 (100일)**: http://localhost:3000/practice/stock/scenario-100days
- **온보딩**: http://localhost:3000/onboarding
- **가이드**: http://localhost:3000/guide
...
#### 재생 속도 조절
게임 화면에서 재생 속도 섹션에서 선택:
- **빠름** (30초/일): 10일 = 5분 | 90일 = 45분
- **보통** (60초/일): 10일 = 10분 | 90일 = 1.5시간 ⭐ 권장
- **느림** (90초/일): 10일 = 15분 | 90일 = 2.25시간
- **여유** (120초/일): 10일 = 20분 | 90일 = 3시간
- **천천히** (180초/일): 10일 = 30분 | 90일 = 4.5시간

#### AI 순위 비교
- 실시간으로 5가지 AI와 순위 비교
- 보수왕, 김철수, 균형왕, 박영희, 초공격왕

#### 힌트 시스템
- **💡 간단 힌트** (300P): 빠른 조언
- **💡💡 심층 분석** (500P): 10단계 분석 + 기대값
...
국내외 가상 주식 시장 게임을 조사하여 벤치마킹 문서를 만들어드리겠습니다. 먼저 관련 정보를 검색하겠습니다.

[7 tools called]

이제 수집한 정보를 바탕으로 벤치마킹 문서를 작성하겠습니다.
...
## 작성된 문서 위치
`document/BENCHMARKING_ANALYSIS.md`
...
**해외 앱 (4개)**
1. **Trading Game** - 400만+ 사용자 (가장 성공적)
2. **Investopedia** - 300만+ 사용자
3. **Three Investeers** - 100만+ 다운로드
4. **Simple Stock Simulator** - 소규모

### 🏆 사용자들이 가장 선호하는 요소 Top 7

1. **실시간 실제 데이터** - 가장 중요!
2. **AI 개인화 및 코칭** - Trading Game의 핵심 성공 요인
3. **경쟁 및 랭킹 시스템** - 10분 배틀 모드가 특히 인기
4. **체계적인 교육 콘텐츠** - 150개 레슨 등
5. **게임화(미션, 보상)** - 지속 참여 유도
6. **직관적한 UI/UX** - 초보자 접근성
7. **다양한 자산군** - 주식+외환+암호화폐

### 🎮 게임 동작 방식

문서에서 5가지 게임 모드를 상세히 분석했습니다:
- 학습 중심 모드 (Trading Game)
- 자유 거래 모드 (Stock'er)
- 미션 기반 모드 (Vestin)
- 경쟁 배틀 모드 (10분 타임 어택)
- 시나리오 기반 모드 (차트 맞추기)
...
- 보수적: 10만 다운로드
- 공격적: 50만 다운로드
- 근거: 경쟁 앱 분석 기반 실현 가능한 수치

문서에는 다운로드 수, 평점, 핵심 기능, 사용자 반응, 성공/실패 요인, 수익 모델, 기술 스택 제안까지 모두 포함되어 있습니다!
