...
> 작성일: 2026-09-21 (7차 개정: **주식은 제2의 자산** 관점 추가 — 1.8, 4.5 성향 측정 10/30문항 개편, 12장 태그 확장. 6차 개정: 서버 없음 → **기기 우선 + 최소 서버**로 변경. 파일명도 GAME_SERVERLESS_AI_COMPARE.md 에서 변경)
...
### 1.8 주식은 제2의 자산이다 — 돈에는 자리가 있어야 한다

지금까지의 문제는 전부 "차트 앞에서의 나"였다. 그런데 실전에서 가장 비싼 매도는 차트 때문이 아니라 **생활 때문에** 일어난다.

| 실전 장면 | 흔한 행동 | 실제로 일어나는 일 |
|---|---|---|
| 꽁돈(보너스, 환급금)이 생겼다 | "없던 돈"이라며 전부 핫한 종목에 | 없던 돈도 잃으면 똑같이 아프다. 추격매수와 겹친다 |
| 갑자기 목돈이 필요하다 (보증금, 병원비, 수리비) | 주식을 판다. 하필 −12%인 날에 | **파는 날을 내가 고르지 못한다.** 손실이 그대로 확정된다 |
| 〃 | 수익 난 종목만 골라 판다 | 처분효과. 계좌에 물린 종목만 남는다 |
| 〃 | 마이너스통장·카드로 막고 "본전 올 때까지" 버틴다 | 이자는 본전을 기다려 주지 않는다 |
| 여행비·쇼핑비가 필요하다 | +6% 난 종목을 팔아 쓴다 | 주식 계좌가 통장이 된다. 매도 이유가 시장이 아니라 지갑이다 |
| 확신이 든다 | 신용·레버리지로 2배 | 수익도 2배, **버틸 시간은 절반** (1.7의 신용융자 사상 최대) |

주식을 현금처럼 쓰는 사람에게 주식은 자산이 아니라 **변동성이 큰 통장**이다. 그래서 이 프로젝트는 매매 습관과 별개로 **돈을 다루는 습관**을 네 가지로 본다.

| 유형 | 한 줄 | 위험 |
|---|---|---|
| 🏦 칸막이형 | 비상금·생활비·투자금이 나뉘어 있다 | 낮음. 현금이 너무 많이 노는지만 점검 |
| 👛 지갑형 | 주식 계좌를 통장처럼 넣었다 뺐다 한다 | 파는 날을 못 고른다 |
| 🎰 올인형 | 생기는 돈은 전부 주식. 현금은 노는 돈 | 급한 날 저점에서 팔게 된다 |
| 💳 빚투형 | 주식은 안 팔고 빚으로 막거나, 빚으로 더 산다 | 이자와 강제 매도가 시간을 빼앗는다 |

> **돈이 필요한 날은 시장이 좋은 날을 기다려 주지 않는다.** (10.3의 삶의 사건과 같은 문장이다.)
> 급락을 버티는 힘은 멘탈이 아니라 **팔지 않아도 되는 구조**에서 나온다.

이 관점은 세 곳에 반영된다: 입구의 성향 측정(4.5), 캐릭터의 삶의 사건(10.3), 습관 테이블의 자산 태그(12장).
...
### 4.5 입구 — 성향 측정 (핵심 10문항 · 전체 30문항)

첫 판 전에는 기록이 없다. 그동안 거울(7.3)을 대신하는 것이 성향 측정이다. 그래서 문항은 "성격 테스트"가 아니라 **이 문서의 문제 장면들**로 구성한다.

| 구간 | 문항 수 (전체 / 핵심) | 장면 | 문서의 근거 |
|---|---|---|---|
| 감정 기본 | 8 / 3 | 첫 매수 10분 뒤, −8% 출근길, 친구의 3배 수익, 목표 수익 도달, 호재 뉴스 | 1.1, 12장 습관 태그 |
| 금액의 무게 | 1 / 1 | 100만원의 −5%와 1,000만원의 −5% | 1.2 |
| 돌발 시나리오 | 3 / 0 | 대표이사 사퇴 3단계 (급락 → 이유 확인 → 반등) | 4.4 시나리오 A |
| **돈의 자리** | 6 / 4 | 꽁돈 300만원, 보증금 500만원 3단계(마련 → 반등을 봄 → 규칙 재설계), 여행비, 신용 2배 | **1.8**, 10.3 |
| 선택과 기다림 | 2 / 0 | 5칸이 꽉 찼을 때 무엇을 내보내나, 3주 횡보 | 5.5, 8.1 |
| 실전 차트 | 10 / 2 | 급등, −8%, 갭하락, +20%, 작전주 의심, **차트 끝의 떨림**, **내가 팔자마자 반등** | 11장, 3.3 |

선택지마다 세 가지가 기록된다.

| 기록 | 값 | 쓰임 |
|---|---|---|
| 성향 | 분석가 / 도전가 / 안정추구 / 감성 / 침착 | 기록이 적을 때 거울의 초기값 (`n / (n + 30)` 혼합) |
| 습관 태그 | 추격매수, 급락 손절, 조기 익절, 물타기, 꼬리 추종, 이유 없는 관망, 본전 집착, 지루함 매매 | 12장 습관 테이블과 같은 이름. 측정 결과 ↔ 실제 플레이의 차이를 나중에 비교 |
| 자산 태그 | 칸막이 / 지갑 / 올인 / 빚투 | 1.8. 캐릭터 삶의 사건(10.3) 때 "예상한 나"와 "실제 나" 비교 |

**결과 리포트는 보고서 형식, 핵심 먼저.**

```mermaid
flowchart TD
    HERO["성향 카드"] --> SUM["핵심 요약 (항상 보임)<br/>① 투자 성향 ② 돈 다루는 습관 ③ 조심할 습관 + 총평"]
    SUM --> D1["▼ 자산 점검 + 코멘트"]
    SUM --> D2["▼ 내 습관 (뿌리가 되는 두려움, 한 줄 처방) + 코멘트"]
    SUM --> D3["▼ 성향 분포 + 코멘트"]
    SUM --> D4["▼ 능력치 (자금 관리 포함 6종) + 코멘트"]
    SUM --> D5["▼ 성향 상세 분석 10줄"]
    SUM --> MISSION["다음 미션 (성향 2개 + 자산 1개)"]
```

총평의 우선순위: 자산 습관이 위험(지갑·올인·빚투)하면 **매매 습관보다 먼저** 말한다. 돈의 자리가 안 잡힌 사람에게 손절 기술은 두 번째 문제다.

> **측정은 "나는 이럴 것이다"이고, 플레이 기록은 "나는 이랬다"이다.** 둘의 차이가 이 제품이 보여줄 수 있는 가장 개인적인 데이터다.

구현: `expo/src/data/analysis-questions.json`, `expo/src/features/analysis/`. 교육용 결과이며 투자 자문이 아님을 리포트 하단에 표기한다.
...
| 본전 집착 | `posPnl` 이 음수에서 0 근처로 돌아오자마자 매도 | 손실을 확정하기 싫음 |
| 지루함 매매 | 횡보(`vol` 낮음) 중 종목 교체·잦은 매매 | 가만히 있으면 뒤처질까 봐 두려움 |
| 이유 없는 관망 | skip + `waitReason = 모르겠다` (8.1) | 결정하기가 두려움 |
| 관망 | skip | — |

**자산 태그 (1.8)** — 삶의 사건(10.3)으로 돈이 빠져나갈 때의 행동에 붙는다.

| 태그 | 조건 (예) | 성향 측정의 유형 |
|---|---|---|
| 사건 매도 | 지출 사건 발생 턴에 평가손실 상태로 매도 | 지갑형 · 올인형 |
| 수익 종목만 매도 | 사건 때 `posPnl` 양수 종목만 매도 | 지갑형 (처분효과) |
| 현금으로 해결 | 사건 금액 ≤ 보유 현금, 매도 없음 | 칸막이형 |

> **습관의 뿌리는 모두 두려움이다. 이름이 다를 뿐이다.**
...
```mermaid
flowchart LR
    A1["성향 측정<br/>핵심 10 · 전체 30 (4.5)"] --> A2["첫 판<br/>거울은 아직 퀴즈 기반"]
